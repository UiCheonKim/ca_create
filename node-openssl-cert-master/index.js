...
var openssl = function(options) {
			...
			if(validsubject.indexOf(prop) >=0 ) {
				...
				if(typeof(options.subject[prop]) != 'string') {
					for(var i = 0; i <= options.subject[prop].length - 1; i++) {
						...
						console.log(options.subject[prop][i]);
						req.push(i + '.' + prop + ' = ' + options.subject[prop][i]);
					}
				} else {
					console.log("ddddddddddddddddddddd");
					console.log(options.subject[prop]);
		...
		if(options.extensions) {
			...
			for(var ext in options.extensions) {
				...
				if(ext == 'SANs') {
					...
					callback('Invalidddd extension: ' + ext, false);
					return false;
				}
			}
			if(sansatend) {
	...
	this.generateRSAPrivateKey = function(options, callback) {
		...
		if(options.format=='PKCS8') {
			runOpenSSLCommandv2({ cmd: cmd.join(' '), stdin: options.encryption.password}, function(err, out) {
				//console.log(err);
				if(err) {
					callback(err, false, false);
				} else {
			...
			runOpenSSLCommandv2({ cmd: cmd.join(' '), stdin: options.encryption.password}, function(err, outkey) {
				if(err) {
					callback(err, false, false);
				} else {
